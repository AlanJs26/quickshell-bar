...
var go = (search, targets, options) => {
    ...
    if (options?.key) {
        ...
        for (var i = 0; i < targetsLen; ++i) {
            ...
            // MODIFICAÇÃO INICIADA: Lidar com arrays para options.key
            var bestResultForObj = NULL;
            ...
            if (Array.isArray(value)) {
                ...
                for (var j = 0; j < items.length; ++j) {
                    ...
                    if (bestResultForObj === NULL || result._score > bestResultForObj._score) {
                        bestResultForObj = result;
                    }
                }
            } else {
                var target = value;
                ...
                if ((searchBitflags & target._bitflags) === searchBitflags) {
                    bestResultForObj = algorithm(preparedSearch, target);
                }
            }

            if (bestResultForObj === NULL) continue;
            if (bestResultForObj._score < threshold) continue;

            bestResultForObj.obj = obj;
            push_result(bestResultForObj);
        ...
        outer: for (var i = 0; i < targetsLen; ++i) {
            ...
            { // early out based on bitflags
                var keysBitflags = 0
                // MODIFICAÇÃO INICIADA: Lidar com arrays na verificação de bitflags
                for (var keyI = 0; keyI < keysLen; ++keyI) {
                    var key = keys[keyI]
                    var value = getValue(obj, key)
                    if (!value) { tmpTargets[keyI] = noTarget; continue }

                    if (Array.isArray(value)) {
                        var preparedItems = [];
                        for (var j = 0; j < value.length; ++j) {
                            var targetItem = value[j];
                            if (!targetItem) continue;
                            if (!isPrepared(targetItem)) targetItem = getPrepared(targetItem);
                            keysBitflags |= targetItem._bitflags;
                            preparedItems.push(targetItem);
                        }
                        tmpTargets[keyI] = preparedItems.length > 0 ? preparedItems : noTarget;
                    } else {
                        var target = value;
                        if (!isPrepared(target)) target = getPrepared(target);
            ...
            for (var keyI = 0; keyI < keysLen; ++keyI) {
                var preparedValue = tmpTargets[keyI] // Pode ser um target preparado ou um array de targets preparados
                if (preparedValue === noTarget) { tmpResults[keyI] = noTarget; continue }

                // MODIFICAÇÃO INICIADA: Lidar com arrays na execução do algoritmo
                var bestResultForKey = NULL;

                if (Array.isArray(preparedValue)) {
                    var preparedItems = preparedValue;
                    for (var j = 0; j < preparedItems.length; ++j) {
                        var target = preparedItems[j];
                        var result = algorithm(preparedSearch, target, /*allowSpaces=*/false, /*allowPartialMatch=*/containsSpace);
                        if (result !== NULL) {
                            if (bestResultForKey === NULL || result._score > bestResultForKey._score) {
                                bestResultForKey = result;
                            }
                        }
                    }
                } else {
                    var target = preparedValue;
                    bestResultForKey = algorithm(preparedSearch, target, /*allowSpaces=*/false, /*allowPartialMatch=*/containsSpace);
                }

                tmpResults[keyI] = (bestResultForKey === NULL) ? noTarget : bestResultForKey;
                // MODIFICAÇÃO FINALIZADA

                if (tmpResults[keyI] === noTarget) continue;

                // o todo original foi removido pois a nova lógica já substitui o melhor resultado
            ...
            if (containsSpace) {
                ...
                for (let i = 0; i < keysLen; i++) { if (tmpResults[i] !== noTarget) { hasAtLeast1Match = true; break } } // Verificação ajustada
                if (!hasAtLeast1Match) continue
            }

            var objResults = new KeysResult(keysLen)
            for (let i = 0; i < keysLen; i++) { objResults[i] = tmpResults[i] === noTarget ? NULL : tmpResults[i] } // Ajustado para popular com NULL
